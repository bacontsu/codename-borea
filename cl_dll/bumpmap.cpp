	...
	while (fgets(szTex, 256, pFile))
	{
		...
		*pBumpFile = '\0';
		...
		for (int i = 0; i < pWorld->numtexinfo; i++)
		{
			...
			{
				sprintf(szTexPath, "%s/bump/%s", gEngfuncs.pfnGetGameDirectory(), pBumpFile);
...
{
	// fullbright map
	if (!LoadCgProgram(&m_fp_Diffuse, m_cg_fragProfile, "programs/fp_diffuse.cg"))
	{
	...
	if (!LoadCgProgram(&m_fp_Lightmaps, m_cg_fragProfile, "programs/fp_lightmaps.cg"))
	{
	...
	// bumpmapping
	if (!LoadCgProgram(&m_fp_Bump, m_cg_fragProfile, "programs/fp_bump.cg"))
	{
	...
	// fullbright model
	if (!LoadCgProgram(&m_fp_ModelPass0, m_cg_fragProfile, "programs/fp_model_p0.cg"))
	{
	...
	if (!LoadCgProgram(&m_fp_ModelPass1, m_cg_fragProfile, "programs/fp_model_p1.cg"))
	{
